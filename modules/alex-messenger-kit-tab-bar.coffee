	...
	if name != @selected
		for item in @items
			if item.name is name
				...
				item.iconLayer.image = item.iconLayer.selectedIcon if item.iconLayer.selectedIcon
				@selected = item.name
				@updateViews(item)
				...
				item.iconLayer.image = item.iconLayer.icon if item.iconLayer.selectedIcon
